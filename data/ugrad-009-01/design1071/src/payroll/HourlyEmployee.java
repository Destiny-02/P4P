package payroll;
public class HourlyEmployee implements Employable{
	private int _taxID;
	...
	public HourlyEmployee(int taxID, String lastName, String firstName, String employment, double rate, double ytd, String startDate, String endDate, double hours, double deduction){
		_taxID = taxID;
		_lastName = lastName;
	...
	public HourlyEmployee(){
		this(0, "First", "Last", "Hourly", 0, 0, "1970-01-01", "2038-12-31", 0, 0);
	}
	public double getWeekly(){
		double weekly;
		if (_hours > OVER_TIME){
			weekly=  (_hours - OVER_TIME) * DOUBLE_TIME * _rate +
					(OVER_TIME - WORK_WEEK) * TIME_AND_HALF * _rate +
					(WORK_WEEK) * _rate;
		} else if (_hours > WORK_WEEK){
			weekly=  ((_hours - WORK_WEEK) * TIME_AND_HALF * _rate) +
					((WORK_WEEK) * _rate);
		} else {
			weekly=  _hours * _rate;
		}
	...
	public double getPaye(){
		...
		return calc.computeYearTax(getYearSalary())/52;
	}
	...
	public double getYearSalary() {
		return getWeekly() * WEEKS_IN_YEAR;
	}
